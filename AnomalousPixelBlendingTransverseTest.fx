...
float3 BlendingPS(float4 position : SV_Position, float2 texcoord : TEXCOORD) : SV_TARGET {
  ...
  // const float2 blendWeights = float2(CORNER_WEIGHT, TRANSVERSE_WEIGHT_);
  // TODO: Potential band-aid fix for the fact that dark lines dont receive as much blending as they should
  const float2 blendWeights = float2(CORNER_WEIGHT, TRANSVERSE_WEIGHT_ * (3f - 2f * saturate(currentL * 5f)));
  ...
  return lerp(current, result, _BlendingStrength);
}

technique AnomalousPixelBlendingTransverseTest {
  pass Blending
  {
